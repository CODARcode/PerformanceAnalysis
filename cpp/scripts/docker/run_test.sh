...
# run web server
python3 bin/ws_flask_stat.py "0.0.0.0" "5001" "logs/webserver.log" &
sleep 1
...
then
    # run ad modules
    mpirun --allow-run-as-root -n $NMPIS bin/driver $ADIOS_MODE $WORK_DIR/BP $BP_PREFIX $WORK_DIR/BP "tcp://0.0.0.0:5559" >logs/ad.log 2>&1 &
    sleep 1
    # run nwchem
