...
// #else
//         std::string req_msg = "Frame " + std::to_string(iFrame);
//         std::cout << "Send: " << req_msg << std::endl;
//         zmq_msg_t req;
//         //zmq_msg_init_data(&req, data, size, ffn, hint);
//         zmq_msg_init_size(&req, req_msg.size());
//         memcpy(zmq_msg_data(&req), (const void*)req_msg.data(), req_msg.size());
//         zmq_msg_send(&req, socket, 0);
//         zmq_msg_close(&req);

//         // zmq_msg_t rep;
//         // zmq_msg_init(&rep);
//         // zmq_msg_recv(&rep, socket, 0);
// #endif
