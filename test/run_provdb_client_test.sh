...
if [ -f "../bin/provdb_admin" ]; then
    ...
    #Connect via tcp
    #Test 1 client, 1 shard
    echo "Test 1 clients 1 shard ${protocol}"
    rm -rf provdb.*  provider.address* margo_dump*.state
    ...
    mpirun -n 1 --allow-run-as-root --oversubscribe ./mainProvDBclient $(cat provider.address.0) ${shards}
