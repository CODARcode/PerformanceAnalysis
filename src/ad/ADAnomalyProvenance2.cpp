#include <chimbuko/ad/ADAnomalyProvenance2.hpp>
...
ADAnomalyProvenance2::ADAnomalyProvenance2(const ADEventIDmap &event_man): m_perf(nullptr), m_event_man(&event_man), m_monitoring(nullptr), m_metadata(nullptr), m_algo_params(nullptr), m_window_size(5){}
...
void ADAnomalyProvenance2::getStackInformation(nlohmann::json &into, const ExecData_t &call) const{
...
void ADAnomalyProvenance2::getWindowCounters(nlohmann::json &into, const ExecData_t &call) const{
...
void ADAnomalyProvenance2::getGPUeventInfo(nlohmann::json &into, const ExecData_t &call) const{
...
void ADAnomalyProvenance2::getExecutionWindow(nlohmann::json &into, const ExecData_t &call) const{
...
void ADAnomalyProvenance2::getNodeState(nlohmann::json &into) const{
  if(m_monitoring)
    into["node_state"] = m_monitoring->get_json();
}

void ADAnomalyProvenance2::getHostname(nlohmann::json &into) const{
  if(m_metadata)
    into["hostname"] = m_metadata->getHostname();
}

void ADAnomalyProvenance2::getAlgorithmParams(nlohmann::json &into, const ExecData_t &call) const{
  if(m_algo_params)
    into["algo_params"] = m_algo_params->get_algorithm_params(call.get_fid());
}


nlohmann::json ADAnomalyProvenance2::getEventProvenance(const ExecData_t &call,
							const int step,
...
void ADAnomalyProvenance2::getProvenanceEntries(std::vector<nlohmann::json> &anom_event_entries,
